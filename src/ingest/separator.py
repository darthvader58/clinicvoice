...
def _get_model(model_name: str):
    """Lazy-load ConvTasNet. Imports asteroid only at first call to keep
    import-time fast and let the rest of the system run without it."""
    global _separator_model, _asteroid_import_err
    if _separator_model is not None:
        return _separator_model

    try:
        from asteroid.models import ConvTasNet
    except Exception as exc:
        _asteroid_import_err = type(exc).__name__
        raise RuntimeError(
    ...
    _separator_model = ConvTasNet.from_pretrained(model_name)
...
def align_stems_to_speakers(
    ...
    """Map each diarization speaker_id to the stem whose RMS energy
    dominates within that speaker's turn windows."""
